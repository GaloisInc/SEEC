...
(define (init-state b-i h-i)
  (if (< h-i 1)
      ...
      (state (repeat (heap-model 0) b-i) hp+ (heap-model null)))))
...
;                 (5) 39 sec found ((copy 0 3) ((copy 1 2) ((free 1))))
; works with d+3** (4) 28s (set 2 6)
;                  (5) 39sec found (set 3 2) (write 1 3)
...
;                 (5) 97 sec found (free 1) (read 3 3) (set 2 6)
...
(define (arbitrary-fp-spec bl)
  (lambda (s s+)
    (let* ([val (nth (state->buf s) bl)])
      (equal? val (state->pointer s+)))))
...
; Gadget is (set 1 (hl-1); write 3 1)
